  ...
  * [database init](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init "database init")
...
## In this article
  * [Synopsis](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#synopsis)
  * [Description](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#description)
  * [Options](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#options)
...
## [Description](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#description)
[Plumbing] Create an empty CodeQL database.
Create a skeleton structure for a CodeQL database that doesn't have a raw QL dataset yet, but is ready for running extractor steps. After this command completes, run one or more [codeql database trace-command](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-trace-command) commands followed by [codeql database finalize](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-finalize) to prepare the database for querying.
...
## [Options](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#options)
### [Primary Options](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#primary-options)
#### [`<database>`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#database)
...
#### [`-s, --source-root=<dir>`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#-s---source-rootdir)
...
#### [`--[no-]overwrite`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--no-overwrite)
...
#### [`--[no-]force-overwrite`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--no-force-overwrite)
...
#### [`--codescanning-config=<file>`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--codescanning-configfile)
...
#### [`--[no-]db-cluster`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--no-db-cluster)
...
#### [`-l, --language=<lang>[,<lang>...]`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#-l---languagelanglang)
...
#### [`--build-mode=<mode>`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--build-modemode)
...
#### [`--[no-]allow-missing-source-root`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--no-allow-missing-source-root)
[Advanced] Proceed even if the specified source root does not exist.
#### [`--[no-]begin-tracing`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--no-begin-tracing)
[Advanced] Create some scripts that can be used to set up "indirect build tracing," which allows integration into existing build workflows when an explicit build command is not available. For information about when and how to use this feature, please refer to our documentation at [Preparing your code for CodeQL analysis](https://docs.github.com/en/code-security/codeql-cli/getting-started-with-the-codeql-cli/preparing-your-code-for-codeql-analysis).
### [Baseline calculation options](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#baseline-calculation-options)
#### [`--[no-]calculate-baseline`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--no-calculate-baseline)
...
#### [`--[no-]sublanguage-file-coverage`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--no-sublanguage-file-coverage)
...
### [Extractor selection options](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#extractor-selection-options)
#### [`--search-path=<dir>[:<dir>...]`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--search-pathdirdir)
...
### [Options to configure how to call the GitHub API to auto-detect languages.](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#options-to-configure-how-to-call-the-github-api-to-auto-detect-languages)
#### [`-a, --github-auth-stdin`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#-a---github-auth-stdin)
Accept a GitHub Apps token or personal access token via standard input.
This overrides the GITHUB_TOKEN environment variable.
#### [`-g, --github-url=<url>`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#-g---github-urlurl)
...
### [Options to configure the package manager.](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#options-to-configure-the-package-manager)
#### [`--registries-auth-stdin`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--registries-auth-stdin)
...
### [Options to configure Windows tracing](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#options-to-configure-windows-tracing)
#### [`--trace-process-name=<process-name>`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--trace-process-nameprocess-name)
[Windows only] When initializing tracing, inject the tracer into a parent process of the CodeQL CLI whose name matches this argument. If more than one parent process has this name, the one lowest in the process tree will be selected. This option overrides `--trace-process-level`, so if both are passed, only this option will be used.
#### [`--trace-process-level=<process-level>`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--trace-process-levelprocess-level)
[Windows only] When initializing tracing, inject the tracer this many parents above the current process, with 0 corresponding to the process that is invoking the CodeQL CLI. The CLI's default behavior if no arguments are passed is to inject into the parent of the calling process, with some special cases for GitHub Actions and Azure Pipelines.
### [Options to configure indirect build tracing](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#options-to-configure-indirect-build-tracing)
#### [`--no-tracing`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--no-tracing)
[Advanced] Do not trace the specified command, instead rely on it to produce all necessary data directly.
#### [`--extra-tracing-config=<tracing-config.lua>`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--extra-tracing-configtracing-configlua)
...
### [Options to control extractor behavior: only be applied to the indirect tracing environment](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#options-to-control-extractor-behavior-only-be-applied-to-the-indirect-tracing-environment)
#### [`-O, --extractor-option=<extractor-option-name=value>`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#-o---extractor-optionextractor-option-namevalue)
...
#### [`--extractor-options-file=<extractor-options-bundle-file>`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--extractor-options-fileextractor-options-bundle-file)
...
When passed to [codeql database init](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init) or `codeql database begin-tracing`, the options will only be applied to the indirect tracing environment. If your workflow also makes calls to [codeql database trace-command](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-trace-command) then the options also need to be passed there if desired.
See <https://codeql.github.com/docs/codeql-cli/extractor-options> for more information on CodeQL extractor options, including how to list the options declared by each extractor.
### [Common options](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#common-options)
#### [`-h, --help`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#-h---help)
Show this help text.
#### [`-J=<opt>`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#-jopt)
[Advanced] Give option to the JVM running the command.
(Beware that options containing spaces will not be handled correctly.)
#### [`-v, --verbose`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#-v---verbose)
Incrementally increase the number of progress messages printed.
#### [`-q, --quiet`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#-q---quiet)
Incrementally decrease the number of progress messages printed.
#### [`--verbosity=<level>`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--verbositylevel)
[Advanced] Explicitly set the verbosity level to one of errors, warnings, progress, progress+, progress++, progress+++. Overrides `-v` and `-q`.
#### [`--logdir=<dir>`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--logdirdir)
[Advanced] Write detailed logs to one or more files in the given directory, with generated names that include timestamps and the name of the running subcommand.
(To write a log file with a name you have full control over, instead give `--log-to-stderr` and redirect stderr as desired.)
#### [`--common-caches=<dir>`](https://docs.github.com/en/code-security/codeql-cli/codeql-cli-manual/database-init#--common-cachesdir)
